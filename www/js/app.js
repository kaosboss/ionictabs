...
angular.module('starter', ['ionic', 'firebase', 'ngSanitize', 'ionic.ion.imageCacheFactory', 'angular-timeline', 'ngImageAppear', 'angular-scroll-animate', 'ion-floating-menu', 'ionic.contrib.ui.tinderCards', 'starter.controllers', 'starter.services'])
  ...
  .run(function ($ionicPlatform, $rootScope, $ionicHistory, $window, $ImageCacheFactory, $state) {
    ...
    $ionicPlatform.ready(function () {
      ...
      $rootScope.closestRI = "Quinta pedagógica";
...
//Callback function when the file has been moved successfully - inserting the complete path
  ...
  .controller("BarCodeReaderController", function ($rootScope, $scope, $cordovaBarcodeScanner, $ionicSlideBoxDelegate, perguntas, $stateParams, $state, $regioes, $timeout, $gameFactory, $ionicHistory) {
    ...
    $scope.initALL = function () {
      if (!$rootScope.fromPI) {
        $scope.isAuto = !$gameFactory.qrHelpValue();
        if ($scope.isAuto)
          $scope.getBarcode();
      } else {
        $scope.isAuto = !$gameFactory.qrHelpValue();
        // $scope.restart = true;
        $rootScope.fromPI = false;
        // $ionicHistory.goBack();
        $state.go("tab.mapa", {
          RI: "ALL",
          PI: ""
    ...
    $scope.getBarcode = function (HELP) {
      console.log("Calling scanbarcode", HELP);
      if (HELP) {
        if (HELP == "NOHELP") {
          $gameFactory.qrHelpValue("OFF");
        }
      }
      $rootScope.deviceBUSY = 1;
      var opts = {
  ...
  .controller('MapaCtrl', function ($scope, $rootScope, $state, $ionicPlatform, $regioes, $stateParams, $timeout, $window, $gameFactory, $ionicScrollDelegate, $ionicTabsDelegate, $ionicHistory) {
      ...
      $scope.goAllQR = function () {
        console.log("Go ALL QR");
        // if ($stateParams.RI != "ALL") {
        //   $ionicTabsDelegate.select(3);
        //   $timeout(function () {
      ...
      $ionicPlatform.ready(function () {
          ...
          for (var f = 0; f <= aCircles.length - 1; f++) {
            ...
            if (dist < circleRadius) {
              ...
              if (aCircles[f].locked) {
                ...
                // if (($scope.regiao.completed) && ($gameFactory.isShowOn($scope.regiao.nome)) && (!$scope.regiao.quizDone)) {
                //   $gameFactory.setShowOff($scope.regiao.nome);
                //   $rootScope.$broadcast('SHOW_QUIZ', {regiao: $regioes.convertRegiaoShortToLong($scope.regiao.nome)});
                // }
        ...
        drawCircle = function (oCircle) {
          ...
          img.onload = function () {
            ...
            // img.style.opacity = "0";
            // context.restore();
            if ($rootScope.closestRI) {
              if (regioes[$rootScope.closestRI] == oCircle.nome) {
        ...
        views: {
          'tab-mapaPIdesafios': {
            templateUrl: function ($stateParams) {
              ...
              if ($stateParams.tipo == "QRALL") {
                console.log(' returned templates/regioes/RI_ALL_QR.html');
                return 'templates/regioes/ALL/RI_ALL_QR.html';
              }
